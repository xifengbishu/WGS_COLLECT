...
# ------------------------------------------------
  ...
  output_file=` date +"%C%y%m%d%H" -d ''${beg_day}' '${beg_hour}' '${int}' hours' `
  ...
  echo "=================="
  # ------
  wgrib2 $wgrib2_file | grep $input_file | grep "${int} hour fcst" | wgrib2 -i $wgrib2_file -grib_out temp_file.grib2 > msg
  # --- do interpolation
